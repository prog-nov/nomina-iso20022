...
class TablasISRquincenal(models.Model):
    _name = 'tablas.isr.quincenal'

    form_id = fields.Many2one('tablas.cfdi', string='ISR Quincenal', required=True)
    lim_inf = fields.Float('Límite inferior') 
    c_fija = fields.Float('Cuota fija') 
    s_excedente = fields.Float('Sobre excedente (%)')
